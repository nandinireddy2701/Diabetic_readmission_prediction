...
st.markdown("**Yeshiva University | DAV Capstone Project | Nandini Reddy Basupally**")
st.markdown("---")
...
page = st.sidebar.radio(
    "Select Page",
    ["Patient Risk Predictor", "Model Performance", "SHAP Feature Importance", "Patient Risk Overview"]
)
...
if page == "Patient Risk Predictor":
    ...
    with col3:
        ...
        admission_type_id        = st.selectbox("Admission Type", [1, 2, 3],
                                                format_func=lambda x: {1: "Emergency", 2: "Urgent", 3: "Elective"}[x])
        discharge_disposition_id = st.selectbox("Discharge Disposition", [1, 2, 3, 4],
                                                format_func=lambda x: {1: "Home", 2: "Care Facility", 3: "Skilled Nursing", 4: "Other"}[x])
        admission_source_id      = st.selectbox("Admission Source", [1, 7],
                                                format_func=lambda x: {1: "Physician Referral", 7: "Emergency Room"}[x])
    ...
    with col4:
        ...
        A1c_normal   = st.selectbox("A1C Normal", [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
        A1c_elevated = st.selectbox("A1C Elevated", [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
        on_insulin   = st.selectbox("On Insulin", [0, 1], format_func=lambda x: "No" if x == 0 else "Yes")
    ...
    med_complexity_score = num_medications * 0.4441 + num_procedures * 0.0018 + number_diagnoses * 0.5541
    high_med_burden      = 1 if num_medications >= 18 else 0
    multiple_med_changes = 0
    meds_x_stay          = num_medications * time_in_hospital
    age_x_diagnoses      = age * number_diagnoses
    composite_risk       = high_med_burden + high_diagnosis_burden + long_stay + emergency_admission
    ...
    if st.button("🔮 Predict Readmission Risk", type="primary"):
        payload = {
            ...
            "med_complexity_score": med_complexity_score,
            "high_med_burden": high_med_burden,
            "on_insulin": on_insulin,
            "multiple_med_changes": multiple_med_changes,
            "meds_x_stay": meds_x_stay,
            "age_x_diagnoses": age_x_diagnoses,
        ...
        bg_map     = {"High": "#ffcccc", "Medium": "#fff3cc", "Low": "#ccffcc"}
...
# ============================================================
    ...
    with col5:
        st.metric("Thesis Confirmed", "+0.0259", "Multi-source vs Clinical-only")
...
# ============================================================
    ...
    col3, col4, col5 = st.columns(3)
    with col3:
        st.image("explainability/shap_summary_bar.png", caption="SHAP Summary Bar")
    with col4:
        st.image("explainability/shap_beeswarm.png", caption="SHAP Beeswarm")
    with col5:
        st.image("explainability/shap_waterfall.png", caption="SHAP Waterfall")
...
# ============================================================
    ...
    st.dataframe(high_risk[['patient_id', 'risk_score', 'risk_tier',
